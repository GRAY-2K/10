...
function displayFormResult(row) {
   if (!row) {
       document.getElementById("formResult").innerHTML = "No results found";
       document.getElementById("formResult").style.display = "block";
       return;
   }

   let formHTML = '<h3 class="history-title">EQUIPMENT DETAILS</h3><div class="equipment-details-container"><table class="details-table">';

   const leftFields = [
       "BME", "TITLE", "MODEL", "MANUFACTURER", "SERIAL", "SITE", "DEPARTMENT", "AREA",
       "RISK CLASS", "ELECTRICAL CLASS TYPE", "ELECTRICAL DATA", "MONTH OF PPM", "PPM FREQUENCY"
   ];

   const rightFields = [
       "STATUS", "VENDOR", "VENDOR CONTACTS", "CONTRACT STATUS", "CONTRACTOR", 
       "CONTRACTOR CONTACTS", "CONTRACT START DATE", "CONTRACT END DATE", "ACCEPTANCE",
       "LAST PPM DATE", "PPM DUE DATE", "WARRANTY START DATE", "WARRANTY END DATE"
   ];
   ...
   if (historyData?.length > 0) {
       const matchingHistoryRecords = historyData.filter(record => 
           record[0] && record[0].toString().toLowerCase() === row["BME"].toString().toLowerCase()
       );

       if (matchingHistoryRecords.length > 0) {
           formHTML += generateHistorySection(matchingHistoryRecords);
       }
   }

   document.getElementById("formResult").innerHTML = formHTML;
   document.getElementById("formResult").style.display = "block";

   // Setup history navigation if history exists
   if (document.querySelector("[data-records]")) {
       setupHistoryNavigation();
   }
}

function generateHistorySection(records) {
   const record = records[0];
   return `
       <h3 class="history-title">EQUIPMENT HISTORY</h3>
       <div class="history-container">
           <div data-records='${JSON.stringify(records)}' data-current-page="0">
               <table class="history-table">
                   ...
                   <tr><th colspan="4">PROBLEM</th></tr>
                   <tr><td colspan="4" class="text-left">${record[4] || ""}</td></tr>
                   <tr><th colspan="4">WORK DETAILS</th></tr>
                   <tr><td colspan="4" class="text-left">${record[5] || ""}</td></tr>
               </table>
               <div style="display: flex; justify-content: space-between; align-items: center; margin-top: 1rem;">
                   ...
                   <span class="page-indicator" style="font-size: 0.875rem;">1/${records.length}</span>
                   <div style="display: flex; gap: 0.5rem;">
...
function setupHistoryNavigation() {
   ...
   function updateButtons() {
       ...
       const prevBtnText = currentPage === records.length - 1 ? "Previous" : "Previous >";
       ...
       nextBtn.style.opacity = currentPage === records.length - 1 ? "0.5" : "1";
